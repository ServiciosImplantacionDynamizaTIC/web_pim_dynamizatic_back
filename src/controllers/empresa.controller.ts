...
export class EmpresaController {
  ...
  async vistaEmpresaMoneda(@param.filter(Empresa) filter?: Filter<Object>,): Promise<Object[]> {
    const dataSource = this.empresaRepository.dataSource;
    const camposSelect = "*"
    const registros = await SqlFilterUtil.ejecutarQuerySelect(dataSource, 'vista_empresa_moneda', filter, camposSelect);
    return registros;
  };
  ...
  async vistaEmpresaMonedaCount(@param.where(Empresa) where?: Where<Empresa>,): Promise<Empresa[]> {
    const dataSource = this.empresaRepository.dataSource;
    const resultado = await SqlFilterUtil.ejecutarQueryCount(dataSource, 'vista_empresa_moneda', where);
    return resultado;
  }
